/**
 * Created by nuthan on 22/08/17.
 */
/**
 * Created by vthodime on 22/08/2017.
 */
...
function add_images() {
    ...
    for (var i = 1; i < 13; i++) {
        ...
        if (this_num > Object.keys(db.img).length) {
    ...
    $.getJSON('https://api.dribbble.com/v1/shots?per_page=100&access_token=d7b35bf8f107cf7310e1291a3ab25fe50bb9a7ade70ee8e46170bd4d4b256087&callback=?', function (resp) {
        if (resp.data.length > 0) {
            $.each(resp.data, function (i, val) {
                db.img[i] = {
                    src: val.images.hidpi,
                    desc: val.description,
                    title: val.title,
                    author: val.user.name,
                    author_pic: val.user.avatar_url,
                    link: val.html_url
                }
            });
        }
